---
slug: "test-blog"
title: "Test Blog"
description: "Markdown is a lightweight markup language for formatting text. It enables you to write in plain text syntax and turn it into structurally correct HTML."

href: "/test-blog"
image: '/nextjs.png'
date: "Mar 06, 2024"
category: "Next.js"

author: Deepak Negi
intro: "Hey! I'm Deepak. I'm a full stack developer, bootcamp grad & Dev."
github: "https://github.com/in-deepaknegi"
insta: "https://www.instagram.com/deepaknegi_00/"
---

## Creating a Next.js project

To embark on your Next.js journey, the first step is setting up your project. Begin by installing Next.js using your preferred package manager. You can use npm or yarn, depending on your workflow. Once installed, create a new Next.js project by running a simple command in your terminalsdfsdfsdfsdfsdfsd.